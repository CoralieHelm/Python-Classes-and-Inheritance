...
#Task 1 The class, Pokemon, is provided below and describes a Pokemon and its leveling and evolving characteristics. An instance of the class is one pokemon that you create.
...
#Task 2. Modify the Grass_Pokemon subclass so that the attack strength for Grass_Pokemon instances does not change until they reach level 10.
...
class Pokemon(object):
    ...
    def train(self):
        self.update() #boost the attack, defense, and health.
        self.attack_up() #so attack up method will boost our Pokemon's attack power by 3
        self.defense_up() #defense up method will boost our Pokemon's defense power by 2
        self.health_up() #health up method will boost our Pokemon's health up by 6
        self.level = self.level + 1 #this will add another level to our Pokemon
        if self.level%self.evolve == 0: #tests if this Pokemon has evolved to level 10.
            return self.level, "Evolved!"
        else:
            return self.level

    def attack_up(self):
        self.attack = self.attack + self.attack_boost #adds a boost to the attack number
        return self.attack
...
#Write another method in the parent class that will be inherited by the subclasses. Call it opponent. It should return which type of pokemon the current type is weak and strong against, as a tuple.
