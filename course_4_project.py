...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        if self.prizeMoney >= VOWEL_COST:
            unguessed = [letter for letter in LETTERS if letter not in guessed]
        else:
            unguessed = [letter for letter in LETTERS if
                        letter not in guessed and letter not in VOWELS]
        return unguessed
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        self.obscuredPhrase = obscuredPhrase
        self.guessed = guessed
        unguessed = self.getPossibleLetters(guessed)
        sortedAndFiltered = [letter for letter in self.SORTED_FREQUENCIES if letter in unguessed]
        if unguessed == []:
            return 'pass'
        else:
            if self.smartCoinFlip() == True:
                goodchoice = sortedAndFiltered[-1]
                return goodchoice
            else:
                randchoice = random.choice(unguessed)
                return randchoice
